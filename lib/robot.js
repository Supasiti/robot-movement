...
// turn right
const turnRight = (posX, posY, facing, times) => {
  if (facings.includes(facing)){
    const newIndex = (facings.indexOf(facing) + times) % 4 ;
    const newFacing = facings[newIndex];
    return robotFactory(posX, posY, newFacing);
  }
  console.error(`facing must be any string in this list [${facings.join(', ')}]`);
  return ;
}
...
const robotFactory = (posX, posY,facing) => {
  if (validateArg(posX,posY,facing)) {
    return {   
      ...
      turnLeft: () => turnRight(posX, posY, facing, 3),
      turnRight: () => turnRight(posX, posY, facing, 1)
    }
  }
  return undefined;
}
